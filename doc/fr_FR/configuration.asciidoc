...
* *Nom expediteur* : nom de l'éxpéditeur du mail (ex : Jeedom)
* *Mail expéditeur* : mail de l'éxpediteur (ex : jeedom@moi.fr)
* *Mode d'envoi* : mode d'envoi du mail : 
** SMTP : mode le plus courant pour l'envoi de mail
** Sendmail (mode expert)
** Qmail (mode expert)
** Mail() (mode expert) : utilise la méthode d'envoi standard de PHP, nécessite d'avoir configuré le l'OS
...
En dessous (partie mail), vous pouvez ajouter des commandes qui correspondent au addresse mail à qui vous souhaitez pouvoir envoyer des mails avec Jeedom : 
